gaussian = read.table("/home/ratanond/Desktop/Masters_Project/Synthetic/Eunji/rnaseq_cls/rnaseq_cls/out/tst/params_sb_high_1_ma_tst.txt",sep = ",")
gaussian <- as.matrix(gaussian)
gaussian_0 <- gaussian[1:50,]
gaussian_1 <- gaussian[501:550,]


library(PMA)
set.seed(1105)
ccaPerm_old = CCA.permute(x = gaussian_0, z = gaussian_1,
                          typex = "standard", typez = "standard", 
                          nperms = 30, niter = 5, standardize = T)
penXtemp = ccaPerm_old$bestpenaltyx
penZtemp = ccaPerm_old$bestpenaltyz
ccaRslt_old = CCA(x = gaussian_0, z = gaussian_1,
                  typex = "standard", typez = "standard",
...
ccaScoreU_old = gaussian_0 %*% ccaRslt_old$u
ccaScoreV_old = gaussian_1 %*% ccaRslt_old$v
...
ccaScores_old$type = c(rep("class 1", 500), rep("class 2", 0))


library(ggplot2)
myCCAPlot = function(x = U1, y = U2, col = V1, shape = type, data = ccaScores_old,
                     xyName = "gaussian_0", coloName = "gaussian_1",
...
myCCAPlot()
myCCAPlot(U1, V1, U1, xyName = "gaussian_1", coloName = "gaussian_0")
